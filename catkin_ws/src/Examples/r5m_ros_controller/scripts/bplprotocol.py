...
class BplSerial():
    ...
    def _init_motors(self, num_motors=6):
        ...
        for num in range(0, num_motors):
            ...
            self.motor_data[motor_name]['deviceID'] = self.device_ids[num]
...
# In bplprotocol.py, 2 functions to send relevant packets
    def send_end_eff_vel(self, DeviceID, x, y, z, yaw=0, pitch=0, roll=0):
        txDeviceId = DeviceID
        txData = ([float(x), float(y), float(z), float(yaw), float(pitch), float(roll)])
        txPacketId = PacketID.KM_END_VEL
        self.serial_device.sendpacket(txDeviceId, txPacketId, txData)

    def send_end_eff_pos(self, DeviceID, x, y, z, yaw=0, pitch=0, roll=0):
        txDeviceId=DeviceID
        txData = ([float(x), float(y), float(z), float(yaw), float(pitch), float(roll)])
        #txData = ([float(x), float(y), float(z)])#20200910
