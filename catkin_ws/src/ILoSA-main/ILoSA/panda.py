...
class Panda():
    ...
    def ee_pose_callback(self, cmd_velocity_message):
        self.cart_pos = [cmd_velocity_message[0],cmd_velocity_message[1], cmd_velocity_message[2]]
    ...
    def connect_ROS(self):
        ...
        rospy.Subscriber("r5m_0/cmd_velocity",  single_float, self.ee_pose_callback)
        ...
        self.goal_pub  = rospy.Publisher('/equilibrium_pose', single_float, queue_size=10)
    ...
    def set_attractor(self,pos):
        goal = single_float()
    
        goal.stamp.secs =  pos[0]
        goal.stamp.nsecs =  pos[1]

        goal.device_id = pos[2]
        goal.value = pos[3]
    ...
    def go_to_3d(self,goal_):
        ...
        w=np.linspace(start[3], goal_[3], step_num)
        print(x,y,z,w)
        position=[x[0],y[0],int(z[0]),w[0]]
       # orientation=[1,0,0,0]
        self.set_attractor(position)
    ...
    def Kinesthetic_Demonstration(self, trigger=0.005): 
       ...
       # self.Passive()
        ...
        while vel < trigger:
            vel = math.sqrt((self.cart_pos[0]-init_pos[0])**2 + (self.cart_pos[1]-init_pos[1])**2 + (self.cart_pos[2]-init_pos[2])**2+(self.cart_pos[3]-init_pos[3])**2)
