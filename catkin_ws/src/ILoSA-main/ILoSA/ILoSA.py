...
class ILoSA(Panda):
    ...
    def Record_NullSpace(self):
        ...
        if save_demo.lower()=='y':
            if len(self.nullspace_traj)==0:
                self.nullspace_traj=np.zeros((4,1))
    ...
    def Record_Demonstration(self):
        ...
        if save_demo.lower()=='y':
            if len(self.training_traj)==0:
                self.training_traj=np.zeros((4,1))
    ...
    def Train_GPs(self):
        if len(self.training_traj)>0 :
            ...
            self.traj=InteractiveGP(X=self.training_traj, y_lim=[-self.attractor_lim, self.attractor_lim], kernel=kernel, n_restarts_optimizer=20)
            self.traj.fit()
            with open('models/delta.pkl','wb') as delta:
                pickle.dump(self.traj,delta)
    ...
    def find_alpha(self):
        alpha=np.zeros(len(self.traj.X))
        for i in range(len(self.traj.X)):         
            pos= self.traj.X[i,:]+self.traj.length_scales 
            dSigma_dx, dSigma_dy, dSigma_dz = self.traj.var_gradient(pos.reshape(1,-1))                                                                                                                                                                
    ...
    def Interactive_Control(self, verboose=False):
        ...
        while not self.end:
            ...
            [self.delta, self.sigma]=self.traj.predict(cart_pos)
            ...
            dSigma_dx, dSigma_dy, dSigma_dz, dSigma_dw= self.traj.var_gradient(cart_pos)
            
            f_stable=-self.alpha*np.array([dSigma_dx, dSigma_dy, dSigma_dz,dSigma_dw])
           ...
           # self.K_tot = np.clip(np.add(self.dK, self.K_mean), self.K_min, self.K_max)
            ...
            if any(np.abs(np.array(self.feedback)) > 0.05): # this avoids to activate the feedback on noise joystick
                ...
                print("dK_inc")
                print(dK_inc)
                is_uncertain=self.traj.is_uncertain(theta=self.theta)
                self.traj.update_with_k(x=cart_pos, mu=self.delta, epsilon_mu=delta_inc, is_uncertain=is_uncertain)
            ...
            #self.scaling_factor = (1- self.sigma / self.traj.max_var) / (1 - self.theta_stiffness)
            ...
            w_new = cart_pos[0][3] + self.delta[3]

            

            pos_goal=[x_new, y_new, z_new,w_new]
            self.set_attractor(pos_goal)
