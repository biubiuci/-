...
class InteractiveGP():
    def __init__(self, X, kernel, y_lim, alpha=1e-10, n_restarts_optimizer=20):
        self.y_lim=y_lim
    ...
    def var_gradient(self, x):
        ...
        dKdw= -2* k_star * (self.X[:, 2].reshape(-1, 1) - x[0][3]) / (lscale[3] ** 2)
        ...
        dSigma_dw_ = np.matmul(a, np.reshape(dKdw, [len(a[0]), 1]))

        return float(dSigma_dx_), float(dSigma_dy_), float(dSigma_dz_),float(dSigma_dw_)
